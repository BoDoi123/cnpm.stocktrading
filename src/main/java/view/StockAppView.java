...
public class StockAppView {
    ...
    private void showAllStocksWindow(List<Stock> stocks, List<StockPrice> stockPrices) {
        ...
        for (Stock stock : stocks) {
            ...
            // Kiểm tra nếu quanity là 0, không thêm vào bảng
            if (stockPrice.getQuantity() > 0) {
    ...
    private void handleViewPersonalStock() {
        List<PersonalStock> personalStocks = stockController.getPersonalStock(userId);
    ...
    private void showPersonalStockWindow(List<PersonalStock> personalStocks) {
        ...
        for (PersonalStock personalStock : personalStocks) {
            // Kiểm tra nếu quanity là 0, không thêm vào bảng
            if (personalStock.getQuantity() > 0) {
    ...
    private void handleViewTransactionHistory() {
        try {
            ...
            List<StockTransaction> transactions = stockController.getTransactionHistory(userId);
    ...
    private void handleLogout() {
        ...
        if (result == JOptionPane.YES_OPTION) {
            ...
            new App(new UserController(new UserDAO()));
        }
    }
}